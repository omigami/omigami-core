...
def test_local_predictions(small_payload, big_payload):
    path = str(ASSETS_DIR / "10k_model/python_model.pkl")
    with open(path, "rb") as input_file:
        local_model = pickle.load(input_file)

    matches_small = local_model.predict(model_input_and_parameters=small_payload, context="")
    matches_big = local_model.predict(model_input_and_parameters=big_payload, context="")
