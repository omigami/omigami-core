        ...
        if len(results_pubchem) > 0:
            ...
            if likely_has_inchi(inchi):
                (
                    ...
                    inchikey_pubchem,
                    smiles_pubchem,
                ) = find_pubchem_inchi_match(results_pubchem, inchi)
            ...
            if inchikey_pubchem is None:
                (
                    ...
                    inchikey_pubchem,
                    smiles_pubchem,
                ) = find_pubchem_mass_match(results_pubchem, parent_mass)
...
def likely_inchi_match(inchi_1, inchi_2, min_agreement=3):
    ...
    if min_agreement == 2:
        print(
            "Warning! 'min_agreement' == 2 has little discriminative power",
...
def likely_inchikey_match(inchikey_1, inchikey_2, min_agreement=1):
    ...
    if (
        len(inchikey_1_parts) >= min_agreement
        and len(inchikey_2_parts) >= min_agreement
    ):
...
def pubchem_name_search(compound_name: str, name_search_depth=10, verbose=1):
    ...
    results_pubchem = pcp.get_compounds(
        compound_name, "name", listkey_count=name_search_depth
    )
    if verbose >= 2:
        print(
            "Found at least", len(results_pubchem), "compounds of that name on pubchem."
        )
...
def pubchem_formula_search(compound_formula: str, formula_search_depth=25, verbose=1):
    ...
    sids_pubchem = pcp.get_sids(
        compound_formula, "formula", listkey_count=formula_search_depth
    )
    ...
    if verbose >= 2:
        print(
            f"Found at least {len(results_pubchem)} compounds of with formula: {compound_formula}."
        )
    return results_pubchem


def find_pubchem_inchi_match(results_pubchem, inchi, min_inchi_match=3, verbose=1):
    ...
    for result in results_pubchem:
        ...
        match_inchi = likely_inchi_match(
            inchi, inchi_pubchem, min_agreement=min_inchi_match
        )
        ...
        if match_inchi:
            ...
            if verbose >= 1:
                print(
                    f"Found matching compound for inchi: {inchi} (Pubchem: {inchi_pubchem}"
                )
            break
...
def find_pubchem_mass_match(
    results_pubchem, parent_mass, mass_tolerance=2.0, verbose=1
):
    """Searches pubmed matches for inchi match.
    Then check if inchi can be matched to (defective) input inchi.
    ...
    for result in results_pubchem:
        ...
        if match_mass:
            ...
            if verbose >= 1:
                print(
                    f"Matching molecular weight ({pubchem_mass:.1f} vs parent mass of {parent_mass:.1f})"
                )
            break
